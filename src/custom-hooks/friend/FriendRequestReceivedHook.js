...
const FriendRequestReceivedHook = () => {
  ...
  const run = async () => {
    setLoading(true);
    setIsPress(true);
    await dispatch(getFriendsRequestReceived(`page=1&limit=4`));
    setLoading(false);
    setIsPress(false);
  };
  useEffect(() => {
  ...
  const getUsersByPagination = async (num) => {
    setLoading(true);
    setIsPress(true);
    await dispatch(getFriendsRequestReceived(`page=${num}&limit=4`));
    setLoading(false);
    setIsPress(false);
  };

  const response = useSelector(
    (state) => state.friend.getFriendsRequestReceived
  );
  let users = [];
...
export default FriendRequestReceivedHook;
